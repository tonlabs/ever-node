...
use ever_block::{
    error, fail, read_boc, read_single_root_boc,
...
impl CollatorTestBundle {
    ...
    pub async fn build_with_ethalon(
        ...
        // prev states
        //
        let mut prev_blocks_ids = vec!(prev.clone());
        if let Some(merge_block_id) = merge_block_id {
            ...
            Self::add_simplified_state(state.root_cell(), &mut state_proofs, &merge_block_id, Some(&block), Some(&usage_tree), None)?;
            Self::add_simplified_state(other.root_cell(), &mut state_proofs, &prev, Some(&block), Some(&usage_tree), None)?;
            prev_blocks_ids.push(merge_block_id);
        } else if !is_master {
            Self::load_and_simplify_state(engine, &mut state_proofs, &prev, Some(&block)).await?;
        }
    ...
    fn get_messages(&self, remp: bool) -> Result<Vec<(Arc<Message>, UInt256)>> {
        let remp_enabled = self.load_state_internal(&self.index.last_mc_state)?
...
pub async fn try_collate(
    ...
    let nodes = compute_validator_set_cc(
        &mc_state,
        &shard,
        prev_blocks_ids.iter().map(|id| id.seq_no()).max().unwrap() + 1,
        cc_seqno_from_state,
